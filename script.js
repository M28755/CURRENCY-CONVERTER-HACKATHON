...
function renderCurrencyList(filter = '') {
    ...
    Currencies.forEach((currency) => {
        ...
        row.innerHTML = `
                <div class="flag"><img src="${currency.flag}" alt="${currency.code}"></div>
...
function getFlagEmoji(currencyCode) {
    ...
    // Map common currency codes to their respective flag country codes
    ...
    const countryCode = currencyToCountry[currencyCode.toUpperCase()] || currencyCode.substring(0, 2).toUpperCase();
...
function updateButtonUI(button, code) {
    ...
    if (flagEl) {
        flagEl.innerHTML = currency ? `<img src="${currency.flag}" alt="${code}">` : '';
    }
...
async function fetchHistoryData() {
    ...
    if (amountInput.value === '0' || amountInput.value === '') {
        if (statsGrid && chartContainer && message) {
...
function currencyValidation() {
    if (currentFromCurrency === currentToCurrency) {
        showToast("Please select different currencies to compare", "error");
        ...
        amountInput.value = ""
        receiveInput.value = ""
        rateSubtext.textContent = `1 USD = 0.8530 EUR`;
...
async function updatestats(todayDate) {
    ...
    try {
        ...
        // console.log(openRate, currentExchangeRate);

        const change = currentExchangeRate - openRate;
        const percentChange = openRate > 0 ? (change / openRate) * 100 : 0;

        document.getElementById('open-stat').textContent = openRate.toFixed(4);
        document.getElementById('last-stat').textContent = currentExchangeRate.toFixed(4);
        ...
        percentElement.textContent = (percentChange >= 0 ? '+' : '') + percentChange.toFixed(4) + '%';
...
document.querySelector('.favorite-btn').addEventListener('click', () => {
    ...
    if (amountInput.value === '0' || amountInput.value === '') {
        showToast('Enter amount to add to favorites', 'error');
        return;
    }
    else {
        savefavorite()
...
document.querySelector('.log-btn').addEventListener('click', () => {
    ...
    if (amountInput.value === '0' || amountInput.value === '') {
        showToast('Enter amount to add to logs', 'error');
        return;
    }
    else {
        const logEntry = {
...
async function fetchCompareRates() {
    ...
    const compareHeaderEl = document.querySelector('.compare-header');

    if (amountInput.value === '0' || amountInput.value === '') {
        if (compareHeaderEl) compareHeaderEl.style.display = 'none';
        compareListEl.innerHTML = `
                <h3 style="color: var(--neutral-100); text-align: center; margin-top: 2rem;">No comparison available</h3>
                <p style="color: var(--neutral-200); text-align: center; font-size: 0.9rem; line-height: 1.5; margin-top: 0.5rem;">Enter an amount in SEND above to see what your <br> money is worth in other currencies.</p>
            `;
        return;
    } else {
        if (compareHeaderEl) compareHeaderEl.style.display = '';
        compareListEl.innerHTML = '<h3 style="color: var(--neutral-500); text-align: center;">Loading rates...</h3>';

        // Update the Header text dynamically
        const baseAmountEl = document.getElementById('compare-base-amount');
        const baseCurrencyEl = document.getElementById('compare-base-currency');
        if (baseAmountEl) baseAmountEl.textContent = amount.toLocaleString();
        if (baseCurrencyEl) baseCurrencyEl.textContent = currentFromCurrency;
        ...
        try {
            ...
            results.forEach(({ code, rate }) => {
                ...
                <div class="compare-row-left">
                    <span class="flag"><img src="${flag}" alt="${code}"></span>
                ...
                <div class="compare-row-right">
                    <div class="converted-group">
                        ...
                        <span class="compare-rate">@ ${rate.toFixed(2)}</span>
                    </div>
...
async function renderFavorites() {
    ...
    if (favorites.length === 0) {
        favoritesList.innerHTML = `
            ...
            <h3 style="color: var(--neutral-100); text-align: center; padding: 40px;">No Pinned Pairs Yet.</h3>
            <p style="color: var(--neutral-100); text-align: center; padding: 40px;">Pin a pair to track its rate here. <br> Tap the star icon on any conversion or comparison row.</p>

            `;
        return;
    }

    favoritesList.innerHTML = '<h3 style="color: var(--neutral-100); text-align: center; padding: 40px;">Loading live rates...</h3>';
...
function renderLogs() {
    ...
    if (logs.length === 0) {
        ...
        logList.innerHTML = `<h3 style="color: var(--neutral-100); text-align: center; padding: 40px;">No Conversions Logged Yet</h3>
               <p style="text-align: center; font-size: 12px; color: var(--neutral-400), word-wrap: break-word">Every conversion is recorded here automatically when you tap LOG CONVERSION. <br> Your log is private to this session and this browser.</p> 
